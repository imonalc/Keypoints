...
def main(args):
    ...
    for img in progress_bar:
        ...
        if Nmatches > 8:
            ...
            MS_list.append(sum(inlier_idx) / len(kpts))
            MMA_list.append(sum(inlier_idx) / Nmatches)
    ...
    print(f"Average; FPS: {ave_fps:.1f}, Matches: {np.mean(Nmatches_list):.1f}", 
          f"MS: {np.mean(MS_list):.3f},MMA: {np.mean(MMA_list):.3f}")
    if len(frames_list) > 0:
        height, width, _ = frames_list[0].shape
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # コーデックの指定
        video_writer = cv2.VideoWriter(f'./results/tracking/{args.descriptor}.mp4', fourcc, ave_fps, (width, height))
...
def get_model(descriptor):
    ...
    if descriptor == 'orb':
        ...
        model = ALIKED(model_name="aliked-n32", # aliked-t16, aliked-n16, aliked-n16rot, aliked-n32
                          device="cuda",
                          top_k=-1,
                          scores_th=0.2,
                          n_limit=feature_limit
