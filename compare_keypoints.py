...
def computes_superpoint_keypoints(img, opt, nms_dist=4, conf_thresh = 0.015, nn_thresh =0.7, cuda = False):
    ...
    fe = train_sp.SuperPointFrontend(weights_path = 'models/superpoint-trained.pth.tar', nms_dist= nms_dist, conf_thresh = conf_thresh, nn_thresh= nn_thresh, cuda = cuda )
...
def keypoint_equirectangular(img, opt ='spoint', crop_degree=0):
    """
...
def main():
    ...
    pts1, desc1 = process_image_to_keypoints(args.path, corners, scale_factor, base_order, sample_order, args.opt, 'tangent')

    pts2, desc2 = process_image_to_keypoints(args.path, corners, scale_factor, base_order, sample_order, args.opt, 'erp')


    #sphorb_points = np.loadtxt("./puntos.dat")
    #sphorb_points[:,0] = (sphorb_points[:,0]/1280)*1024
    #sphorb_points[:,1] = (sphorb_points[:,1]/640)*512
    #print(sphorb_points)

    ind1 = np.argsort(pts1[:,2].numpy(),axis = 0)[::-1]
    ind2 = np.argsort(pts2[:,2].numpy(),axis = 0)[::-1]

    max1 = np.min([args.points,ind1.shape[0]])
    max2 = np.min([args.points,ind2.shape[0]])

    ind1 = ind1[:max1]
    ind2 = ind2[:max2]

    erp_kp = pts2[ind2.copy(),:]
    tangent_image_kp = pts1[ind1.copy(),:]
    tangent_image_kp = pts2[ind2.copy(),:]#sphorb_points
    print(erp_kp.shape)
    print(tangent_image_kp.shape)

    desc1 = desc1[:,ind1.copy()]
    desc2 = desc2[:,ind2.copy()]

    img = load_torch_img(args.path)[:3, ...].float()
    img = F.interpolate(img.unsqueeze(0), scale_factor=scale_factor, mode='bilinear', align_corners=False, recompute_scale_factor=True).squeeze(0)
